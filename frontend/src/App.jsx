...
const POS_URL = 'http://localhost:8501';
  ...
  useEffect(() => {
    ...
    const fetchAdminData = async () => {
      ...
      try {
        ...
        const [productsRes, staffRes, salesRes, ordersRes, blogRes, sectionsRes, usersRes] = await Promise.all([
        ...
        const [products, staff, sales, orders, blogPosts, sections, users] = await Promise.all([
          productsRes.json(),
          staffRes.json(),
          salesRes.json(),
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      ...
      if (authMode === 'register') {
        ...
        if (data.reset_token) {
          setAuthNotice(`Reset token: ${data.reset_token}`);
          setAuthForm({ ...authForm, resetToken: data.reset_token });
          setAuthMode('reset');
        } else {
          setAuthNotice('If the email exists, a reset token has been issued.');
        }
  ...
  const handleAdminCreateProduct = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        buying_price: adminProductForm.buying_price ? Number(adminProductForm.buying_price) : null,
    ...
    return (
      <div key={product.id} className="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer relative">
        ...
        <div onClick={() => { setSelectedProduct(product); setView('product'); }} className="p-4">
          <div className="text-xs text-gray-500 uppercase mb-1 font-semibold">{product.brand}</div>
          <h3 className="font-bold text-lg mb-1 group-hover:text-red-600 transition">{product.model}</h3>
            ...
            {featuredProducts.map(product => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3">
                    <div className="text-xs text-gray-500 uppercase mb-1">{product.brand}</div>
                    <h3 className="font-semibold text-sm mb-1 truncate">{product.model}</h3>
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <div className="space-y-6">
              <div>
                <div className="text-sm text-gray-500 uppercase mb-2 font-semibold">{selectedProduct.brand}</div>
                <h1 className="text-4xl font-bold mb-3">{selectedProduct.model}</h1>
                ...
                {cart.map(item => (
                  <div key={item.id} className="bg-white rounded-xl p-4 sm:p-6 shadow-md flex items-center space-x-4 sm:space-x-6">
                    ...
                    <div className="flex-1">
                      <h3 className="font-bold text-base sm:text-lg">{item.product.brand} {item.product.model}</h3>
            ...
            <div className="space-y-3 mb-6 max-h-64 overflow-y-auto">
              {cart.map(item => (
                <div key={item.id} className="flex justify-between text-sm pb-3 border-b">
                  <div className="flex-1">
                    <div className="font-medium">{item.product.model}</div>
